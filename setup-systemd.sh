...
# このスクリプトは以下を実行します:
#   1. プロジェクトディレクトリの確認
#   2. サービスファイル (config/hxs-reservation-bot.service) の確認
#   3. バイナリのビルド
#   4. 環境変数 (.env) の確認
#   5. サービスファイルを /etc/systemd/system/ にコピー
#   6. systemd の設定を反映
#   7. サービスの有効化と起動
#
# 注意:
#   - 実行前に .env ファイルを設定してください
#   - または、サービスファイルに直接環境変数を記述してください
#   - サービスファイル内の User, WorkingDirectory, ExecStart は
#     実際の環境に合わせて編集してください
#
